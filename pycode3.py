...
def model_pdct(data1, row, ipts, paras, output1):
    ...
    if len(ipts) == 4:
        ...
        pdct_rst_flat = model.predict(pdct_data).reshape(21*21*21*21,layers[-1])
        exact_rst_flat = output1(pdct_data).reshape(21*21*21*21,layers[-1])
        error_rst_flat = np.abs(pdct_rst_flat - exact_rst_flat)
        pdct_rst = pdct_rst_flat.reshape(21,21,21,21,layers[-1])
        exact_rst = exact_rst_flat.reshape(21,21,21,21,layers[-1])
        error_rst = error_rst_flat.reshape(21,21,21,21,layers[-1])
    elif len(ipts) == 3:
        ...
        pdct_rst_flat = model.predict(pdct_data).reshape(21*21*21,layers[-1])
        exact_rst_flat = output1(pdct_data).reshape(21*21*21,layers[-1])
        error_rst_flat = np.abs(pdct_rst_flat - exact_rst_flat)
        pdct_rst = pdct_rst_flat.reshape(21,21,21,layers[-1])
        exact_rst = exact_rst_flat.reshape(21,21,21,layers[-1])
        error_rst = error_rst_flat.reshape(21,21,21,layers[-1])
    elif len(ipts) == 2:
        ...
        pdct_rst_flat = model.predict(pdct_data).reshape(21*21,layers[-1])
        exact_rst_flat = output1(pdct_data).reshape(21*21,layers[-1])
        error_rst_flat = np.abs(pdct_rst_flat - exact_rst_flat)
        pdct_rst = pdct_rst_flat.reshape(21,21,layers[-1])
        exact_rst = exact_rst_flat.reshape(21,21,layers[-1])
        error_rst = error_rst_flat.reshape(21,21,layers[-1])
    elif len(ipts) == 1:
        pdct_data = np.concatenate((np.transpose(np.mgrid[ipts[0][0]:ipts[0][1]:21j].reshape(1,21**1)), np.tile(paras, (21**1, 1))), axis=1)
        pdct_rst_flat = model.predict(pdct_data).reshape(21,layers[-1])
        exact_rst_flat = output1(pdct_data).reshape(21,layers[-1])
        error_rst_flat = np.abs(pdct_rst_flat - exact_rst_flat)
        pdct_rst = pdct_rst_flat.reshape(21,layers[-1])
        exact_rst = exact_rst_flat.reshape(21,layers[-1])
        error_rst = error_rst_flat.reshape(21,layers[-1])
